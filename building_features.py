...
def main():
    ...
    for (i, (trainPath, cleanedPath)) in enumerate(imagePaths):
        ...
        cleanImage = cleanedImage.astype("float") / 255.0
        ...
        for y in range(0, trainImage.shape[0]):
            for x in range(0, trainImage.shape[1]):
                ...
                # clean image, then grab the spatial dimensions of the ROI
    ...
    # close the CSV file
    pbar.finish()
    print(f"[INFO] features CSV file: {config.FEATURES_PATH}")
    csv.close()
